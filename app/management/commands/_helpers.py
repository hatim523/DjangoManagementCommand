...
def create_pst_datetime():
    ...
    return pytz.timezone(zone='US/Pacific').localize(random_date)
...
def load_date_with_timezone_info(date_object: DateStore) -> datetime.datetime:
    return date_object.date.replace(tzinfo=pytz.timezone(date_object.timezone))


def add_pst_datetimes(count=100):
    for i in range(count):
        datetime_object = create_pst_datetime()
        DateStore.objects.create(
            date=datetime_object,
            timezone=datetime_object.tzinfo.zone,
        )
